...
class ViewMovies extends View{
	...
	public function output(){
		...
		echo"<form  action='storeinventory.php?action=insert' method='post'>";		
	...
	$con = new mysqli("localhost", "root", "","pharmacy");
  	...
  	$result = mysqli_query($con,$sql);	
		...
		echo"<select style='width:120px;'  name='InventoryID'>";
	...
	echo"</form>";
		...
		foreach($this->model->getMovies() as $Movie){
			...
			$str.="<td>".$Movie->getInventoryID() ."  </td> ";
			$str.="<td>
			<a class='fas fa-edit' href='storeinventory.php?action=edit&id=".$Movie->getID()."'></a>
			</td>";
			$str.="<td>
			<a  style='color:red;' class='fas fa-trash-alt' href='storeinventory.php?action=delete&id=".$Movie->getID()."'></a>
			</td>";
			$str.="</tr>";
		}
	...
	public function edit($id){
		...
		foreach($this->model->getMovies() as $Movie){
			if($Movie->getID()===$id) 	{
				$str.="<tr>";
				$str.="<form action='storeinventory.php?action=editAction&id=".$Movie->getID()."' method='post'>";
				$str.="<td>" . $Movie->getID() ."</td> ";
				$str.="<td><input style='width:80px;' type='text' name='StoreID' value='". $Movie->getStoreID() ."'></td> ";
				$str.="<td><input style='width:80px;' type='text' name='InventoryID' value='".$Movie->getInventoryID() ."'>  </td> ";
			...
			$str.="<td>" . $Movie->getID() ."</td> ";
				...
				$str.="<td><a class='fas fa-edit' href='storeinventory.php?action=edit&id=".$Movie->getID()."'></a></td>";
				$str.="</tr>";
			}
		}
		$str.="</table>";
