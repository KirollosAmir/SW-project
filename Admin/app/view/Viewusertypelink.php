...
class ViewMovies extends View{
	...
	public function output(){
		...
		echo"<form  action='usertypelink.php?action=insert' method='post'>";		
	...
	$con = new mysqli("localhost", "root", "","pharmacy");
  	...
  	$result = mysqli_query($con,$sql);	
		...
		echo"<select style='width:120px;' name='UserID'>";
	...
	$con = new mysqli("localhost", "root", "","pharmacy");
  	...
  	$result = mysqli_query($con,$sql);	
		...
		echo"<select style='width:120px;' name='LinkID'>";
	...
	echo"</form>";
		...
		foreach($this->model->getMovies() as $Movie){
			...
			$str.="<td>".$Movie->getUserID() ."  </td> ";
			$str.="<td>" . $Movie->getLinkID() ."</td> ";
			
			$str.="<td>
			<a class='fas fa-edit' href='usertypelink.php?action=edit&id=".$Movie->getID()."'></a>
			</td>";
			$str.="<td>
			<a  style='color:red;'  class='fas fa-trash-alt' href='usertypelink.php?action=delete&id=".$Movie->getID()."'></a>
			</td>";
			$str.="</tr>";
		}
	...
	public function edit($id){
		...
		foreach($this->model->getMovies() as $Movie){
			if($Movie->getID()===$id) 	{
				$str.="<tr>";
				$str.="<form action='usertypelink.php?action=editAction&id=".$Movie->getID()."' method='post'>";
				$str.="<td>".$Movie->getID() ."  </td> ";
				$str.="<td><input style='width:80px;' type='text' name='UserID' value='".$Movie->getUserID() ."'>  </td> ";
				$str.="<td><input style='width:80px;' type='text' name='LinkID' value='". $Movie->getLinkID() ."'></td> ";
				...
				$str.="<td><a class='fas fa-edit' href='usertypelink.php?action=edit&id=".$Movie->getID()."'></a></td>";
				$str.="</tr>";
			}
		}
		$str.="</table>";
