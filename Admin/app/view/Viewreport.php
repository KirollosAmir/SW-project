...
class ViewMovies extends View{
	...
	public function output(){
		...
		$con = new mysqli("localhost", "root", "","pharmacy");
				...
				echo"<form action='report.php?action=insert' method='post'>";		
                echo"<table style='margin-left:-30px;' class='table table-bordered' >";

						
			echo"<tr> ";
			echo"	<td>ReportName</td>";
				echo"<td><input style='width:120px;' type='text' name='ReportName' required='' maxlength='15' onkeypress='return a(event);' ></td>";
	...
	echo"</form>";
		...
		foreach($this->model->getMovies() as $Movie){
			...
			$str.="<td>" . $Movie->getReportName() ."</td> ";
			$str.="<td>".$Movie->getUserTypeID() ."  </td> ";
			
			$str.="<td>
			<a  class='fas fa-edit' href='report.php?action=edit&id=".$Movie->getID()."'></a>
			</td>";
			$str.="<td>
			<a  style='color:red;' class='fas fa-trash-alt'  href='report.php?action=delete&id=".$Movie->getID()."'></a>
			</td>";
			$str.="</tr>";
		}
	...
	public function edit($id){
		...
		foreach($this->model->getMovies() as $Movie){
			if($Movie->getID()===$id) 	{
				$str.="<tr>";
				$str.="<form action='report.php?action=editAction&id=".$Movie->getID()."' method='post'>";
				$str.="<td>" . $Movie->getID() ."</td> ";
				$str.="<td><input style='width:80px;' type='text' name='ReportName' value='". $Movie->getReportName() ."'></td> ";
			...
			$str.="<td>" . $Movie->getID() ."</td> ";
				...
				$str.="<td><a class='fas fa-edit' href='report.php?action=edit&id=".$Movie->getID()."'></a></td>";
				$str.="</tr>";
			}
		}
		$str.="</table>";
