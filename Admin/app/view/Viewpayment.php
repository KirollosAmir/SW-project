...
class ViewMovies extends View{
	...
	public function output(){
		...
		//$str.="<form action='payment.php?action=insert' method='post'>";
		
		$con = new mysqli("localhost", "root", "","pharmacy");
		
				echo"<form name='myForm' onsubmit='return validateForm() action='payment.php?action=insert' method='post'>";		
                echo"<table style='margin-left:-30px;' class='table table-bordered' >";

						
			echo"<tr> ";
			echo"	<td>Name</td>";
				echo"<td><input onkeypress='return a(event);' required='' style='width:120px;'  type='text' name='Name' ></td>";
	...
	echo"</form>";
		...
		foreach($this->model->getMovies() as $Movie){
			...
			$str.="<td>
			<a class='fas fa-edit' href='payment.php?action=edit&id=".$Movie->getID()."'></a>
			</td>";
			$str.="<td>
			<a   style='color:red;' class='fas fa-trash-alt' href='payment.php?action=delete&id=".$Movie->getID()."'></a>
			</td>";
			$str.="</tr>";
		}
	...
	public function edit($id){
		...
		foreach($this->model->getMovies() as $Movie){
			if($Movie->getID()===$id) 	{
				$str.="<tr>";
				$str.="<form name='myForm' onsubmit='return validateForm()' action='payment.php?action=editAction&id=".$Movie->getID()."' method='post'>";
				$str.="<td>" . $Movie->getID() ."</td> ";
				$str.="<td><input style='width:80px;' onkeypress='return a(event);' required='' type='text' name='Name' value='". $Movie->getName() ."'></td> ";
			...
			$str.="<td>" . $Movie->getID() ."</td> ";
				...
				$str.="<td><a class='fas fa-edit' href='payment.php?action=edit&id=".$Movie->getID()."'></a></td>";
				$str.="</tr>";
			}
		}
		$str.="</table>";
