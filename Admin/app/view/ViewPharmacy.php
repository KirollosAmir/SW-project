...
class ViewMovies extends View{
	...
	public function output(){
		...
		$con = new mysqli("localhost", "root", "","pharmacy");
				...
				echo"<form   name='myForm' onsubmit='return validateForm()' action='Pharmacies.php?action=insert' method='post'>";		
echo"<table style='margin-left:-30px;' class='table table-bordered' >";
echo"<tr> ";
				echo"<td >Name</td>";
				echo"<td><input style='width:120px;' type='text' maxlength='20' onkeypress='return a(event)'; required='' name='Name' ></td>";
			
			echo"	<td >Location</td>";
				echo"<td><input style='width:120px;' required='' maxlength='50'  type='text' name='Location' ></td>";
	...
	echo"</form>";
		...
		foreach($this->model->getMovies() as $Movie){
			...
			$str.="<td>" . $Movie->getLocation() ."</td> ";
			$str.="<td>" . $Movie->getUserTypeID() ."</td> ";
			
			
			$str.="<td>
			<a class='fas fa-edit' href='Pharmacies.php?action=edit&id=".$Movie->getID()."'></a>
			</td>";
			$str.="<td>
			<a  style='color:red;' class='fas fa-trash-alt' href='Pharmacies.php?action=delete&id=".$Movie->getID()."'></a>
			</td>";
			$str.="</tr>";
		}
	...
	public function edit($id){
		...
		foreach($this->model->getMovies() as $Movie){
			if($Movie->getID()===$id) 	{
				$str.="<tr>";
				$str.="<form action='Pharmacies.php?action=editAction&id=".$Movie->getID()."' method='post'>";
				$str.="<td>".$Movie->getID() ."  </td> ";
				$str.="<td><input style='width:80px;' type='text' name='Name' value='".$Movie->getName() ."'>  </td> ";
				$str.="<td><input style='width:80px;' type='text' name='Location' value='". $Movie->getLocation() ."'></td> ";
				...
				$str.="<td><a class='fas fa-edit' href='Pharmacies.php?action=edit&id=".$Movie->getID()."'></a></td>";
				$str.="</tr>";
			}
		}
		$str.="</table>";
