...
public function output(){
		...
		$con = new mysqli("localhost", "root", "","pharmacy");
				...
				echo"<form  action='Cardriver.php?action=insert' method='post' >";		
echo"<table style='margin-left:-30px;' class='table table-bordered'>";
	...
	echo"<td>";
  	...
  	$result = mysqli_query($con,$sql);	
		...
		echo"<select style='width:120px;' name='CarID' >";
	...
	echo"</form>";
		...
		foreach($this->model->getMovies() as $Movie){
			...
			$str.="<td>" . $Movie->getCarID() ."</td> ";
			
			$str.="<td>
			<a class='fas fa-edit' href='Cardriver.php?action=edit&id=".$Movie->getID()."'></a>
			</td>";
			$str.="<td>
			<a  style='color:red;'  class='fas fa-trash-alt' href='Cardriver.php?action=delete&id=".$Movie->getID()."'></a>
			</td>";
			$str.="</tr>";
		}
	...
	public function edit($id){
		...
		foreach($this->model->getMovies() as $Movie){
			if($Movie->getID()===$id) 	{
				$str.="<tr>";
				$str.="<form action='Cardriver.php?action=editAction&id=".$Movie->getID()."' method='post'>";
			$str.="<td>".$Movie->getID() ."  </td> ";
				$str.="<td><input style='width:80px;' type='text' name='UserTypeID' value='".$Movie->getUserTypeID() ."'>  </td> ";
				$str.="<td><input style='width:80px;' type='text' name='CarID' value='". $Movie->getCarID() ."'></td> ";
				...
				$str.="<td><a  class='fas fa-edit'href='Cardriver.php?action=edit&id=".$Movie->getID()."'></a></td>";
				$str.="</tr>";
			}
		}
		$str.="</table>";
