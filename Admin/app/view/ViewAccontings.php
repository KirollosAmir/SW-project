...
class ViewMovies extends View{
	...
	public function output(){
		...
		//$str.="<form action='Accountings.php?action=insert' method='post'>";
		
		$con = new mysqli("localhost", "root", "","pharmacy");
		
				echo"<form  name='myForm' onsubmit='return validation()'  action='Accountings.php?action=insert' method='post'>";		
echo"<table style='margin-left:-30px;' class='table table-bordered' >";
echo"<tr> ";
				echo"<td style='font-size:20px'>Taxes</td>";
				echo"<td><input style='width:120px;' onkeypress='return isInputNumber(event);' type='text' maxlength='20' required='' name='Taxes' ></td>";
			
			
		
			echo"	<td style='font-size:20px'>delivery</td>";
				echo"<td><input style='width:120px;' onkeypress='return isInputNumber(event);' type='text' name='delivery' ></td>";
	...
	echo"<td>";
  	...
  	$result = mysqli_query($con,$sql);	
		...
		echo"</td>";
			...
			echo"	<td></td>";
			echo"	<td><input style='width:120px;' style='font-size:20px' type='submit' name='submit' value='Submit'></td>";
			echo"</tr>";
	echo"</table>";
	echo"</form>";
		...
		foreach($this->model->getMovies() as $Movie){
			...
			$str.="<td>".$Movie->getTaxes() ."  </td> ";
			$str.="<td>" . $Movie->getdelivery() ."</td> ";
			$str.="<td>" . $Movie->getUserTypeID() ."</td> ";
			
			
	
			$str.="<td>
			<a class='fas fa-edit' href='Accountings.php?action=edit&id=".$Movie->getID()."'></a>
			</td>";
			$str.="<td>
			<a  style='color:red;'  class='fas fa-trash-alt' class='fas fa-trash-alt'  href='Accountings.php?action=delete&id=".$Movie->getID()."'></a>
			</td>";
			$str.="</tr>";
		}
	...
	public function edit($id){
		...
		foreach($this->model->getMovies() as $Movie){
			if($Movie->getID()===$id) 	{
				$str.="<tr>";
				$str.="<form action='Accountings.php?action=editAction&id=".$Movie->getID()."' method='post'>";
				$str.="<td>".$Movie->getID() ."  </td> ";
				$str.="<td><input style='width:80px;' type='text'  onkeypress='return isInputNumber(event);' name='Taxes' value='".$Movie->getTaxes() ."'>  </td> ";
				$str.="<td><input style='width:80px;' type='text'   onkeypress='return isInputNumber(event);' name='delivery' value='". $Movie->getdelivery() ."'></td> ";
				...
				$str.="<td><a class='fas fa-edit' href='Accountings.php?action=edit&id=".$Movie->getID()."'></a></td>";
				$str.="</tr>";
			}
		}
		$str.="</table>";
